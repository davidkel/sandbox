...
class IDManager {
    ...
    async enrollToWallet(userID, secret, mspId, walletToImportTo, labelToUse) {
        if (!labelToUse) {
            labelToUse = userID;
        }
        await walletToImportTo.configureClientStores(this.client, labelToUse);
